...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, val_file=""):
    ...
    # D2：loss 记录到 loss_log.csv
    loss_log_path = os.path.join(scene.model_path, "loss_log.csv")
    if not os.path.exists(loss_log_path):
        with open(loss_log_path, "w") as f:
            f.write("iteration,L1,SSIM_loss,depth_loss,total\n")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # D2：每 10 轮追加记录 {iteration, L1, SSIM loss, depth loss, total}
        if iteration % 10 == 0:
            with open(loss_log_path, "a") as f:
                f.write("{},{:.6f},{:.6f},{:.6f},{:.6f}\n".format(
                    iteration, Ll1.item(), 1.0 - ssim_value.item(), float(Ll1depth), loss.item()))
        ...
        with torch.no_grad():
            ...
            # 新功能：val10 渲染（每 500 轮，含第 1 轮），输出当前轮次 3DGS 的 RGB 渲染图与 GT 副本
            if val_cameras and iteration % 500 == 1:
...
def plot_loss_curves(loss_log_path, model_path, iteration):
    """D2：根据 loss_log.csv 输出损失曲线图（总曲线 + 前 20000 轮放大）"""
    try:
        import matplotlib
        matplotlib.use("Agg")
    ...
    for col, name in [(1, "L1"), (2, "SSIM loss"), (3, "depth loss"), (4, "total")]:
...
def render_val_cameras(val_cameras, gaussians, pipe, background, iteration, model_path, separate_sh, train_test_exp, elapsed_sec=0.0):
    """新功能：渲染全部 val 相机，输出当前轮次的 RGB 渲染图与高斯法向量方向颜色贴图；
    GT 图只在第一轮保存（避免重复占用磁盘）；同时计算 val 集 L1/PSNR/SSIM 并追加到 val_metrics.csv"""
    ...
    # 高斯法向量方向颜色贴图：由旋转四元数得旋转矩阵，取第 3 列（最小缩放轴/法向），RGB=(n+1)/2
    R = build_rotation(gaussians._rotation)
    normal_colors = torch.clamp((R[:, :, 2] + 1.0) * 0.5, 0.0, 1.0).contiguous()
    ...
    for idx, cam in enumerate(val_cameras):
        ...
        # 法向量方向颜色贴图渲染（强制 separate_sh=False，避免 colors_precomp 与 SH 双通道冲突）
        normal_pkg = render(cam, gaussians, pipe, background, 1.0, False, normal_colors, train_test_exp)
        nimg = torch.clamp(normal_pkg["render"], 0.0, 1.0)
