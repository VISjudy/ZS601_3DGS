	...
	float* dL_dopacity)
{
	...
	if (dL_dnormals)
	{
		...
		const float dL_ny = dL_dnormals[idx * 3 + 1];
		const float dL_nz = dL_dnormals[idx * 3 + 2];
		if (dL_nx != 0.f || dL_ny != 0.f || dL_nz != 0.f)
		{
			...
			atomicAdd(&dL_drot_n->z, 2.f * z * dL_ny - 4.f * y * dL_nz);
			atomicAdd(&dL_drot_n->w, 2.f * (x * dL_nx + y * dL_ny));
		}
	}
}
...
)
{
	...
	if (inside)
	{
		...
		if (dL_dnormals)
			for (int i = 0; i < 3; i++)
				dL_dnormal_pix[i] = dL_dout_normals[i * H * W + pix_id];
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (dL_dnormals)
			{
				...
				{
					const float nc = collected_normals[ch * BLOCK_SIZE + j];
					accum_normal_rec[ch] = last_alpha * last_normal[ch] + (1.f - last_alpha) * accum_normal_rec[ch];
					last_normal[ch] = nc;
					dL_dalpha += (nc - accum_normal_rec[ch]) * dL_dnormal_pix[ch];
					atomicAdd(&(dL_dnormals[global_id * 3 + ch]), dchannel_dcolor * dL_dnormal_pix[ch]);
				}
			}
