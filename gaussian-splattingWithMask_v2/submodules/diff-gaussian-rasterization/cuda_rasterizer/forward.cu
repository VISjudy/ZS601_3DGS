	...
	bool antialiasing)
{
	...
	// 若与（高斯中心 - 相机中心）点积为正（背向相机）则翻转，保证法向始终朝向相机
	{
		...
		float y = q.z;
		float z = q.w;
		float nx = 2.f * (x * z + r * y);
		float ny = 2.f * (y * z - r * x);
		float nz = 1.f - 2.f * (x * x + y * y);
		float vx = p_orig.x - cam_pos->x;
		float vy = p_orig.y - cam_pos->y;
		float vz = p_orig.z - cam_pos->z;
		if (nx * vx + ny * vy + nz * vz > 0.f)
		{
			nx = -nx; ny = -ny; nz = -nz;
		}
		normals[idx * 3 + 0] = nx;
	...
	float* __restrict__ out_normal)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (out_normal)
			{
				...
				expected_normal[1] += normals[collected_id[j] * 3 + 1] * alpha * T;
				expected_normal[2] += normals[collected_id[j] * 3 + 2] * alpha * T;
			}
	...
	if (inside)
	{
		...
		if (out_normal)
		{
			...
			out_normal[1 * H * W + pix_id] = expected_normal[1];
			out_normal[2 * H * W + pix_id] = expected_normal[2];
		}
	}
}
