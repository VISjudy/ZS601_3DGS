	...
	const bool debug)
{
  ...
  if(dL_dout_normal.size(0) != 0)
  {
	...
	dL_dnormalsptr = dL_dnormals.data<float>();
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dnormals);
}
