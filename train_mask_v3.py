...
def main(argv=None):
    ...
    assert a.experiment=='original','Paper experiments use the original optimizer/densification baseline'
    assert not any(getattr(a,n) for n in FEATURES if n!='validation_diagnostics'),'Unexpected legacy geometry feature enabled'
    ...
    print('[RUN]',json.dumps({'experiment':a.experiment,'features':{n:getattr(a,n) for n in FEATURES},
          ...
          'final_test':a.final_test,'final_test_iteration':a.iterations,'test_worst_count':10},indent=2),flush=True)
    ...
    from precomputed_v3 import PrecomputedSupervision,supervision_terms
    external=None
    if a.paper_arm!='A':
        # Synthetic filenames encode immutable view IDs; virtual RGB is explicitly unsupervised.
        train_ids={c.image_name:int(Path(c.image_name).stem) for c in cameras}
        virtual=set(json.loads(Path(a.supervision_manifest).read_text())['virtual_names']) if a.paper_arm=='E' else set()
        external=PrecomputedSupervision(a.precomputed_root,{n:i for n,i in train_ids.items() if n not in virtual},cache_size=4,skip_images=virtual)
    depth_dataset=None
    ...
    try:
        for iteration in range(first+1,a.iterations+1):
            ...
            if external is not None:
                ramp=min(1.,max(0.,(iteration-a.supervision_start)/max(a.supervision_warmup,1)))
                ext,ext_stats=supervision_terms(cam,g,pipe,external,
                    depth_weight=a.metric_depth_weight*ramp if a.paper_arm in ('B','D','E') else 0.,
                    normal_weight=a.external_normal_weight*ramp if a.paper_arm in ('C','D','E') else 0.)
                loss=loss+ext
                append_json(out/'supervision_loss.jsonl',dict(iteration=iteration,image=cam.image_name,**ext_stats))
        ...
        if a.final_test=='on' and iteration==a.iterations:
            ...
            print(f'[FINAL TEST] skipped: enabled={a.final_test} completed_iteration={iteration}; target={a.iterations}',flush=True)
    except BaseException as error:
