...
def render_geometry(cam,g,pipe):
    ...
    from precomputed_v3 import depth_to_camera_normals
    fx=cam.image_width/(2*math.tan(cam.FoVx/2));fy=cam.image_height/(2*math.tan(cam.FoVy/2))
    normals,normal_valid=depth_to_camera_normals(depth,alpha,(fx,fy,cam.image_width/2,cam.image_height/2))
...
def _copy_pseudo_gt(a,cam,out,stem):
    ...
    candidates={
        'lidar_depth_gt':root/'depth'/image_path.with_suffix('.png'),
        'lidar_depth_valid':root/'valid'/image_path.with_suffix('.png'),
        'lidar_normal_gt':root/'normal_preview'/image_path.with_suffix('.png'),
        'lidar_normal_valid':root/'normal_valid'/image_path.with_suffix('.png'),
    }
    result={}
...
def export_final_test(a,iteration,cameras,g,pipe,input_identity):
    ...
    from gaussian_renderer import render
    if iteration!=a.iterations:
        raise ValueError('Final test export requires the configured terminal iteration')
    if not cameras:
